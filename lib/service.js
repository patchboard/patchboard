...
Service = (function() {
  ...
  function Service(options) {
    var key, mapping, path_string, resource_type, value, _ref, _ref1, _ref2, _ref3, _ref4;
    ...
    this.resources = {};
    _ref = PatchboardAPI.resources;
    for (key in _ref) {
      value = _ref[key];
      this.resources[key] = value;
    }
    _ref1 = options.resources;
    for (key in _ref1) {
      value = _ref1[key];
      this.resources[key] = value;
    }
    this.paths = {};
    this.directory = {};
    _ref2 = PatchboardAPI.paths;
    for (resource_type in _ref2) {
      mapping = _ref2[resource_type];
      if (mapping.publish) {
        this.directory[resource_type] = "" + this.service_url + mapping.path;
      }
    }
    _ref3 = this.map;
    for (resource_type in _ref3) {
      mapping = _ref3[resource_type];
      if (mapping.publish) {
        this.directory[resource_type] = "" + this.service_url + mapping.path;
      }
    }
    ...
    for (resource_type in _ref4) {
      mapping = _ref4[resource_type];
      path_string = mapping.path;
      this.paths[resource_type] = new Path(path_string);
    }
    this.documenter = new Documenter(this.schema_manager.names, this.resources);
  ...
  Service.prototype.classify = function() {
    var args, _ref;
    args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
    return (_ref = this.classifier).classify.apply(_ref, args);
  };

  Service.prototype.validate = function() {
    var args, _ref;
    args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
    return (_ref = this.validator).validate.apply(_ref, args);
  };
  ...
  Service.prototype.documentation = function() {
    return "" + (this.documenter.document_resources()) + "\n\n" + (this.schema_manager.document());
  };
